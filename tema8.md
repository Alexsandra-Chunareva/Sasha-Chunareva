...
class Book:
    def __init__(self, title, author, year, genre):
        # Инициализация атрибутов книги: название, автор, год, жанр и статус прочтения
        self._title = title
        self._author = author
        self._year = year
        self._genre = genre
    ...
    def display_info(self):
        ...
        print(f"Год издания: {self._year}")
        print(f"Жанр: {self._genre}")
        print(f"Прочитана: {'Да' if self._is_read else 'Нет'}")

    def mark_as_read(self):
        # Запрос у пользователя статуса прочтения книги
        user_input = input(f"Прочитана ли книга '{self._title}'? (Да/Нет): ")
        if user_input.lower() == 'да':
            # Обновление статуса на "прочитана"
            self._is_read = True
            print(f"Книга '{self._title}' отмечена как прочитанная.")
        elif user_input.lower() == 'нет':
            # Обновление статуса на "непрочитана"
            self._is_read = False
            print(f"Книга '{self._title}' отмечена как непрочитанная.")
        else:
            print("Некорректный ввод.")

# Класс для электронных книг, наследует Book
class EBook(Book):
    def __init__(self, title, author, year, genre, file_size, file_format):
        # Инициализация атрибутов книги и добавление уникальных атрибутов для EBook: размер файла и формат
        super().__init__(title, author, year, genre)
        self.__file_size = file_size
        self.__file_format = file_format

    def display_info(self):
        # Расширение метода вывода информации для отображения данных о файле
        super().display_info()
        print(f"Размер файла: {self.__file_size} МБ")
        print(f"Формат файла: {self.__file_format}")

# Класс для аудиокниг, наследует Book
class AudioBook(Book):
    def __init__(self, title, author, year, genre, duration, narrator):
        # Инициализация атрибутов книги и добавление атрибутов для AudioBook: длительность в минутах и чтец
        super().__init__(title, author, year, genre)
        self.__duration_minutes = duration * 60  # Длительность в минутах
        self.__narrator = narrator
        self.__listened_minutes = 0  # Изначально количество прослушанных минут равно 0

    def display_info(self):
        # Расширение метода вывода информации для отображения данных о прослушивании
        super().display_info()
        print(f"Длительность: {self.__duration_minutes // 60} часов {self.__duration_minutes % 60} минут")
        print(f"Чтец: {self.__narrator}")
        print(f"Прослушано: {self.__listened_minutes // 60} часов {self.__listened_minutes % 60} минут")
    ...
    def mark_as_read(self):
        # Запрос у пользователя количества прослушанных минут
        try:
            minutes = int(input(f"Сколько минут вы уже прослушали от книги '{self._title}'? "))
            # Проверка на корректность введенного значения и добавление к прослушанному времени
            if 0 <= minutes <= (self.__duration_minutes - self.__listened_minutes):
                self.__listened_minutes += minutes
                print(f"Обновлено: прослушано {self.__listened_minutes // 60} часов {self.__listened_minutes % 60} минут из {self.__duration_minutes // 60} часов.")
            else:
...
# Создание объекта электронной книги
ebook = EBook("Мастер и Маргарита", "Михаил Булгаков", 1966, "Роман", 2.5, "PDF")
# Создание объекта аудиокниги
audiobook = AudioBook("Преступление и наказание", "Фёдор Достоевский", 1866, "Роман", 20, "Сергей Безруков")

# Цикл для полиморфного вызова методов display_info и mark_as_read для каждого объекта книги
for book in (ebook, audiobook):
    # Вывод информации о книге
    book.display_info()
    # Обновление статуса прочтения или количества прослушанных минут
    book.mark_as_read()
    print()
```
